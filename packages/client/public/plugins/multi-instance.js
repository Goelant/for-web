const mod$1 = window.__STOAT__["solid-js/web"];
// Re-export all named exports
const keys$1 = Object.keys(mod$1);
for (const key of keys$1) {
  // Can't use static export in a loop, so we use a Proxy trick
}
// Use a single destructure that Rollup can tree-shake
const {render: render$1,Portal,Dynamic,template,delegateEvents,insert,spread,effect,memo,style,classList,className,use,createComponent,setAttribute,addEventListener,innerHTML} = mod$1;

const S = window.__STOAT__;
const {
  createSignal,
  createMemo,
  createContext,
...
const { createStore } = S["solid-js/store"];
const { useSmartParams, useParams, useNavigate, useLocation } = S["@revolt/routing"];
const { useClient, clientContext } = S["@revolt/client"];
const { entryContainer } = S["@revolt/app/sidebar"];

const mod = window.__STOAT__["stoat.js"];
// Re-export all named exports
...
// Use a single destructure that Rollup can tree-shake
const {Client,ConnectionState,Channel,Server,User,Message,Collection} = mod;
...
async function tryFetchConfig(baseUrl) {
  try {
...
async function validateInstanceUrl(url) {
  ...
  const bestGuess = normalized.endsWith("/api") ? normalized : normalized + "/api";
...
class ClientManager {
  ...
  constructor(multiAuth, instancesStore) {
    this.multiAuth = multiAuth;
    this.instancesStore = instancesStore;
    const [connectedInstances, setConnectedInstances] = createSignal(
      []
    );
  ...
  updateConnectedList() {
    this.setConnectedInstances(
      Array.from(this.instances.keys()).filter((url) => {
        ...
        return instance && instance.state() !== "disconnected" /* Disconnected */;
      })
    );
  }
  /**
  ...
  connectInstance(instanceUrl) {
    ...
    client.events.on("state", (connState) => {
      switch (connState) {
  ...
  async fetchConfigAndConnect(client, instanceUrl) {
    try {
      ...
      if (managed) {
        managed.setState("error" /* Error */);
        this.updateConnectedList();
      }
    }
  }
  /** Disconnect from an instance */
  disconnectInstance(instanceUrl) {
    const url = normalizeInstanceUrl(instanceUrl);
   ...
   */
  resolveServerInstance(serverId) {
   ...
   */
  resolveChannelInstance(channelId) {
    for (const [url, managed] of this.instances) {
  ...
  allServers() {
    ...
    for (const managed of this.instances.values()) {
      if (managed.state() === "connected" /* Connected */) {
    ...
    for (const managed of this.instances.values()) {
      if (managed.state() === "connected" /* Connected */) {
        for (const channel of managed.client.channels.toList()) {
          if (channel.type === "DirectMessage" && channel.active || channel.type === "Group") {
   ...
   */
  async login(instanceUrl, credentials, friendlyName) {
    const url = normalizeInstanceUrl(instanceUrl);
    try {
      const stoat = window.__STOAT__;
      const APIClass = stoat["stoat.js"].API.API;
...
const STORAGE_KEY$1 = "multi-instance:instances";
...
class MultiAuth {
  ...
  async hydrate(legacySession) {
    ...
    if (Object.keys(this.store.sessions).length === 0 && legacySession?.session) {
  ...
  isValidSession(session) {
    ...
    const s = session;
    return typeof s._id === "string" && typeof s.token === "string" && typeof s.userId === "string" && typeof s.valid === "boolean";
  }
  ...
  async setSession(instanceUrl, session) {
    this.setStore("sessions", normalizeInstanceUrl(instanceUrl), session);
    await this.persist();
  }
  /** Remove session for an instance */
  async removeSession(instanceUrl) {
    this.setStore("sessions", normalizeInstanceUrl(instanceUrl), void 0);
    await this.persist();
  }
  /** Mark a session as valid (after successful WebSocket connection) */
  async markValid(instanceUrl) {
    const url = normalizeInstanceUrl(instanceUrl);
  ...
  getActiveSessions() {
    return Object.entries(this.store.sessions).filter(([, session]) => session && session.valid).map(([instanceUrl, session]) => ({ instanceUrl, session }));
  }
  /** Get all sessions (including not-yet-validated) */
  getAllSessions() {
    return Object.entries(this.store.sessions).filter(([, session]) => session).map(([instanceUrl, session]) => ({ instanceUrl, session }));
  }
}

var _tmpl$$1 = /* @__PURE__ */ template(`<p style=color:var(--md-sys-color-error);margin:0>`), _tmpl$2$1 = /* @__PURE__ */ template(`<div style=display:flex;flex-direction:column;gap:12px><label>Instance URL</label><input placeholder=https://example.com><div style=display:flex;gap:8px;justify-content:flex-end><button style=background:transparent;color:var(--md-sys-color-on-surface)>Cancel</button><button>`), _tmpl$3 = /* @__PURE__ */ template(`<p style=color:var(--md-sys-color-tertiary);margin:0;font-size:12px>Could not validate the instance (CORS). Login may still work.`), _tmpl$4 = /* @__PURE__ */ template(`<div style=display:flex;flex-direction:column;gap:12px><p style=margin:0>Logging into <strong></strong></p><input type=email placeholder=Email><input type=password placeholder=Password><div style=display:flex;gap:8px;justify-content:flex-end><button style=background:transparent;color:var(--md-sys-color-on-surface)>Back</button><button>`), _tmpl$5 = /* @__PURE__ */ template(`<p style=color:var(--md-sys-color-primary)>Connected successfully! Closing...`), _tmpl$6 = /* @__PURE__ */ template(`<div style=position:fixed;top:0;left:0;right:0;bottom:0;z-index:10000;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.5)><div style=background:var(--md-sys-color-surface-container-high);border-radius:16px;padding:24px;min-width:380px;max-width:440px;color:var(--md-sys-color-on-surface)><h2 style="margin:0 0 16px 0;font-size:20px">Add an Instance`);
const solidWeb = window.__STOAT__;
const {
  render
} = solidWeb["solid-js/web"];
function mountAddInstanceModal(manager, show, onClose) {
  const container = document.createElement("div");
  container.id = "multi-instance-modal";
  document.body.appendChild(container);
  render(() => createComponent(AddInstanceOverlay, {
    manager,
    show,
    onClose
  }), container);
...
function AddInstanceOverlay(props) {
  const [step, setStep] = createSignal("url");
  ...
  async function handleLogin() {
    ...
    setLoading(true);
    try {
      const result = await props.manager.login(resolvedApiUrl(), {
        email: email(),
        password: password()
      ...
      if (result.success) {
        ...
        setError(typeof result.error === "object" && result.error !== null && "type" in result.error ? `Error: ${result.error.type}` : String(result.error));
      }
    } catch (err) {
      setError(String(err));
    } finally {
  ...
  return createComponent(Show, {
    ...
    get children() {
      var _el$ = _tmpl$6(), _el$2 = _el$.firstChild; _el$2.firstChild;
      _el$.$$click = (e) => {
      ...
      insert(_el$2, createComponent(Show, {
        ...
        get children() {
          var _el$4 = _tmpl$2$1(), _el$5 = _el$4.firstChild, _el$6 = _el$5.nextSibling, _el$8 = _el$6.nextSibling, _el$9 = _el$8.firstChild, _el$0 = _el$9.nextSibling;
          _el$6.$$keydown = (e) => {
            if (e.key === "Enter") handleValidateUrl();
          };
          _el$6.$$input = (e) => setInstanceUrl(e.currentTarget.value);
          ...
          _el$9.$$click = close;
          _el$0.$$click = handleValidateUrl;
          insert(_el$0, () => loading() ? "Checking..." : "Next");
          effect((_p$) => {
            var _v$ = inputStyle, _v$2 = {
              ...buttonStyle
            }, _v$3 = buttonStyle, _v$4 = loading() || !instanceUrl().trim();
          ...
          effect(() => _el$6.value = instanceUrl());
          return _el$4;
        }
      }), null);
      insert(_el$2, createComponent(Show, {
        ...
        get children() {
          var _el$1 = _tmpl$4(), _el$10 = _el$1.firstChild, _el$11 = _el$10.firstChild, _el$12 = _el$11.nextSibling, _el$14 = _el$10.nextSibling, _el$15 = _el$14.nextSibling, _el$17 = _el$15.nextSibling, _el$18 = _el$17.firstChild, _el$19 = _el$18.nextSibling;
          insert(_el$12, instanceName);
          ...
          _el$14.$$input = (e) => setEmail(e.currentTarget.value);
          _el$15.$$keydown = (e) => {
            if (e.key === "Enter") handleLogin();
          };
          _el$15.$$input = (e) => setPassword(e.currentTarget.value);
          insert(_el$1, createComponent(Show, {
            get when() {
              return error();
            },
          ...
          _el$19.$$click = handleLogin;
          insert(_el$19, () => loading() ? "Logging in..." : "Login");
          effect((_p$) => {
            var _v$5 = inputStyle, _v$6 = inputStyle, _v$7 = {
              ...buttonStyle
            }, _v$8 = buttonStyle, _v$9 = loading() || !email().trim() || !password().trim();
            _p$.e = style(_el$14, _v$5, _p$.e);
            _p$.t = style(_el$15, _v$6, _p$.t);
            _p$.a = style(_el$18, _v$7, _p$.a);
            _p$.o = style(_el$19, _v$8, _p$.o);
            _v$9 !== _p$.i && (_el$19.disabled = _p$.i = _v$9);
            return _p$;
          }, {
            e: void 0,
            t: void 0,
          ...
          effect(() => _el$14.value = email());
          effect(() => _el$15.value = password());
          return _el$1;
        }
      }), null);
      insert(_el$2, createComponent(Show, {
...
var _tmpl$ = /* @__PURE__ */ template(`<svg xmlns=http://www.w3.org/2000/svg viewBox="0 0 24 24"width=24 height=24 fill=currentColor><path d="M20 13H4c-.55 0-1 .45-1 1v6c0 .55.45 1 1 1h16c.55 0 1-.45 1-1v-6c0-.55-.45-1-1-1zM7 19c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zM20 3H4c-.55 0-1 .45-1 1v6c0 .55.45 1 1 1h16c.55 0 1-.45 1-1V4c0-.55-.45-1-1-1zM7 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z">`), _tmpl$2 = /* @__PURE__ */ template(`<a>`);
function DnsIcon() {
  return _tmpl$();
}
const UI = window.__STOAT__;
const Avatar = UI["@revolt/ui"].Avatar;
const Tooltip = UI["@revolt/ui"].Tooltip;
function createMultiInstanceBridge(manager) {
  return function MultiInstanceBridge(props) {
    const params = useParams();
    const primaryController = useContext(clientContext);
    const reactiveController = new Proxy(primaryController, {
      get(target, prop) {
        if (prop === "getCurrentClient") {
          return () => {
            ...
            if (entityId) {
              ...
              const channelUrl = !serverUrl ? manager.resolveChannelInstance(entityId) : void 0;
            ...
            return target.getCurrentClient();
          };
        }
        return target[prop];
      }
...
function createPluginServers(manager, primaryClient) {
  return function PluginServers() {
    ...
    return createComponent(For, {
      ...
      children: (server) => createComponent(Tooltip, {
        placement: "right",
        get content() {
          return server.name;
        },
        ...
        get children() {
          ...
          effect((_p$) => {
            var _v$ = entryContainer(), _v$2 = `/server/${server.id}`;
